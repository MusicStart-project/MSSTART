<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<c:if test="${empty authUser}">
<!-- 현재 URL 가져오는 방법 request.getAttribute("javax.servlet.forward.request_uri") -->
<input type="button" value="로그인" onclick="location.href='/MS/user/login.do?url=/album/index.do'">
</c:if>
<c:if test="${!empty authUser}">
${authUser.name }님 반갑습니다.
<input type="button" value="로그아웃" onclick="location.href='/MS/user/logout.do'">
</c:if>
<input type="button" value="main" onclick="location.href='/MS/';">
<input type="button" value="user" onclick="location.href='/MS/user/index.do';">
<input type="button" value="board" onclick="location.href='/MS/album/index.do';">